...
conf = {
    ...
    'redirect_url': '{}/authorization/redirect?conf=okta'.format(os.environ.get('OKTA_AUDIENCE'))
}
okta_blueprint = OAuth2ConsumerBlueprint('okta', __name__, **conf)

conf.update({
    'scope': os.environ.get('OKTA_ADMIN_SCOPES').split(','),
    'redirect_url': '{}/authorization/redirect?conf=okta-admin'.format(os.environ.get('OKTA_AUDIENCE'))
})
